...
async def lego(event):
 ...
 try:
    ...
    fname2 = "Logo.png"
    img.save(fname2, "png")
    await tbot.send_file(event.chat_id, fname2, caption="Made By @akalankanime")
    if os.path.exists(fname2):
            os.remove(fname2)
 except Exception as e:
   await event.reply(f'Join with us ~  to use this, {e}')
...
async def logo(event):
 ...
 try:
    text = event.pattern_match.group(1)
    img = Image.open('./DewmiBot/resources/0.jpg')
    draw = ImageDraw.Draw(img)
    image_widthz, image_heightz = img.size
    pointsize = 500
    fillcolor = "white"
...
async def lego(event):
 ...
 try:
    text = event.pattern_match.group(1)
    img = Image.open('./DewmiBot/resources/5.jpg')
    draw = ImageDraw.Draw(img)
    image_widthz, image_heightz = img.size
    pointsize = 25
    fillcolor = "gold"
